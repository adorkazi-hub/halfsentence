...
export async function loadCurrentlyReading() {
  const { data, error } = await supabase
    .from('site_settings').select(
      'currently_reading_book, currently_reading_author, currently_reading_quote, currently_reading_progress'
    ).eq('id', 1).maybeSingle()

  if (error || !data) {
    // fallback — animate bar with default
    setTimeout(() => {
...
export async function loadPost() {
  ...
  try {
    ...
    supabase.rpc('increment_views', { post_id: postId }).catch(() => {})
